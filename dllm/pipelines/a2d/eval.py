...
@dataclass
class A2DEvalConfig(MDLMSamplerConfig):
    """Configuration for A2D model evaluation."""
...
@register_model("a2d")
class A2DEvalHarness(LM):
    """A2D model evaluation harness for lm-eval framework."""
    ...
    def __init__(
        self,
        config: A2DEvalConfig | None = None,
        **kwargs,
    ):
        super().__init__()
        if config is None:
            config = A2DEvalConfig()
        ...
        if torch.distributed.is_initialized():
            self._rank = torch.distributed.get_rank()  # ← GLOBAL rank
            self._world_size = (
        ...
        self.model.eval()
        # Disable SDPA to avoid dtype mismatch issues with attention masks in bfloat16
    ...
    def _forward_process(
        ...
        """Forward diffusion process for masking."""
        b, l = batch.shape
    ...
    def get_loglikelihood(self, prefix: torch.Tensor, target: torch.Tensor) -> float:
        """Compute log likelihood for a sequence pair."""
    ...
    def suffix_greedy_prediction(
        ...
        """Check if greedy decoding produces the target suffix."""
    ...
    def _encode_pair(
        ...
        return torch.tensor(context_enc), torch.tensor(continuation_enc)
