...
try:
    ...
    from Dream_utils.generation_utils import DreamGenerationMixin, DreamGenerationConfig
except:
    from mtphead_config import MTPHeadConfig
...
class DreamRotaryEmbedding(nn.Module):
    def __init__(
        ...
        scaling_factor=1.0,
        rope_type="default",
        config: Optional[MTPHeadConfig] = None,
    ):
        super().__init__()
...
class DreamAttention(nn.Module):
    ...
    def __init__(self, config: MTPHeadConfig, layer_idx: Optional[int] = None):
        super().__init__()
        self.config = config
...
class DreamSdpaAttention(DreamAttention):
    ...
    def forward(
        ...
        # Prepare attn_mask for scaled_dot_product_attention.
        # Accept several incoming formats for `attention_mask` and convert to a boolean
        # mask broadcastable to [batch, num_heads, q_len, k_len].
        attn_mask = None
        if isinstance(attention_mask, torch.Tensor):
            am = attention_mask
            # convert numeric masks to boolean (non-zero => True/visible)
            if am.dtype != torch.bool:
                am = am != 0

            # am could be: [B, N] / [B, 1, N] / [B, q, k] / [B, 1, q, k]
            if am.dim() == 2:
                # [B, N] -> [B, 1, N, N]: allow positions that both source and target are visible
                am2 = am.unsqueeze(1)  # [B,1,N]
                am = (am2.unsqueeze(-1) & am2.unsqueeze(-2))  # [B,1,N,N]
            elif am.dim() == 3:
                # [B, q, k] -> [B,1,q,k]
                am = am.unsqueeze(1)
            elif am.dim() == 4:
                # already [B,1,q,k] or [B, num_heads, q, k]
                # ensure shape is [B, num_heads, q, k] or [B,1,q,k]
                pass
            else:
                am = None

            # If am has shape [B,1,q,k] and num_heads > 1, it will be broadcastable.
            attn_mask = am
...
class DreamDecoderLayer(nn.Module):
    def __init__(self, config: MTPHeadConfig, layer_idx: int):
        super().__init__()
        self.hidden_size = config.hidden_size
...
class DreamBaseModel(DreamPreTrainedModel):
    ...
    def forward(
        ...
        # TODO: inputs_embeds here is our condition hidden states
        # so we should concat both token embedding and input embeds
        # -------------------------------------------------------------
...
class DreamModel(DreamGenerationMixin, DreamPreTrainedModel):
    ...
    def topK_genrate(self, 
        ...
        if use_cache:
            ...
            T = x.shape[1]
            tok_idx = torch.arange(0, T , dtype=torch.long, device=hidden_states.device)
            tok_idx = tok_idx.unsqueeze(0).expand(batch_size, -1)  # [B, T]

            mask = torch.zeros(T, T, dtype=torch.bool, device=hidden_states.device)

            # input_ids 区域 (i < N)：只能看 input_ids
            mask[:input_len, :input_len] = True
            # mask 区域 (i >= N)：可以看全部
            mask[input_len:, :] = True
            attention_mask = mask.unsqueeze(0).unsqueeze(1).expand(batch_size, 1, -1, -1)
        else:
        ...
        diffu_time = start_time.elapsed_time(end_time) / 1000  # seconds
        ...
        #         # print(entropy_norm)
        #         dynamic_ks = dynamic_ks.ceil().long().clamp(min=2, max=k_max) 
        #         candidates_list = []
        #         for j in range(top_tokens.size(0)):
        #             k = dynamic_ks[j].item()
        #             print(f"position:{j}, token: {tokenizer.decode(top_tokens[j, :k])}")
        ...
        # print(f"*** draft time ; {diffu_time:.4f} s ***")
        ...
        end_time.record()
        end_time.synchronize()
        tree_time = start_time.elapsed_time(end_time)/1000 - diffu_time  # seconds
        ...
        return draft_tokens, retrieve_indices, tree_mask, tree_position_ids, diffu_time, tree_time
