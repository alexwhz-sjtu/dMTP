...
class Qwen2RotaryEmbedding(nn.Module):
    ...
    def forward(self, x, position_ids):
        ...
        # Core RoPE block
        inv_freq_expanded = self.inv_freq[None, :, None].float().to(x.device).expand(position_ids.shape[0], -1, 1)
        position_ids_expanded = position_ids[:, None, :].float().to(x.device)

        # 直接在 float32 下计算，避免 bfloat16 精度损失（长上下文关键）
        # 去掉 autocast，防止设备错乱
        freqs = torch.matmul(inv_freq_expanded, position_ids_expanded).transpose(1, 2)  # [bs, 1, seq_len]
        emb = torch.cat((freqs, freqs), dim=-1)  # 扩展为 sin/cos 形式
